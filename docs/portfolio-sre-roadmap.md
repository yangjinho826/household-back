...
| 실증 | **다중 인스턴스 멱등성 실증 1회** | 인스턴스 2개가 동일 PostgreSQL 공유 시 동일 키 경합 → 거래 1건(보호 끄면 2건 대조). **1회 실증에 그치지 않고 회귀 테스트 6개로 승격해 CI 게이트 편입** (59→65). 경계 실측: 멱등성·advisory lock ✅ / 알림 쿨다운 ❌(인스턴스별). `docs/multi-instance-proof.md` | ✅ 2026-08-03 |
...
## 실증. 다중 인스턴스 멱등성 — ✅ 완료 (2026-08-03)

- **2층 구성**: 회귀 게이트(호스트 uvicorn 프로세스 2개, `tests/multi_instance/` 6개 — `test.yml` reusable 이라 PR·배포 게이트에 자동 편입) + 운영 구성 재현(`docker-compose.multi.yml`, 운영 이미지·entrypoint→alembic→uvicorn).
- **LB 안 씀** — 라운드로빈은 두 요청이 같은 인스턴스로 갈 수 있어 경합을 보장 못 한다. 주소 지정 = 인스턴스 지정.
- **결론**: 상태를 DB 에 둔 것만 인스턴스 안전. 멱등성·advisory lock ✅ / 알림 쿨다운은 프로세스 메모리라 인스턴스별 ❌ (`alert.py` 주석의 한계를 실측으로 확정).
- **발견 3건** (상세 `docs/multi-instance-proof.md`):
  1. `ddl/init.sql` 이 alembic baseline 에서 드리프트 → 빈 볼륨에서 `init.sql → upgrade head` 체인이 끊긴다(`fixed_expenses.amount`). 운영·재해복구는 무관, **새 환경 구축 경로만 영향**. 실증은 우회했고 근본 수정은 미착수.
  2. 멱등 캐시로 재생된 요청은 access log 에 안 남는다(미들웨어 순서) → 재시도 트래픽 관측 사각.
  3. **HTTP 200 수신이 DB 커밋 가시성을 보장하지 않는다** — 보호 없는 경로에서 즉시 집계가 0/1/2 로 흔들림(최종은 항상 2). 검증을 최종 상태 단언(`_settled_count`)으로 교체.

**감수한 한계 (면접 재료)**: 인스턴스 2개·동일 호스트까지만 / alembic 동시 기동은 no-op 상태에서만 관측 / E3 는 락 의미론이 아니라 배선 검증(락 판정 단위가 커넥션이라) / 발견 3의 메커니즘은 가설.
...
| 숫자 | 마이그레이션 **24** · 도메인 **17** · 태그 배포 **22** · 통합 테스트 **65** · 잡 5개 · AI 로그 183건 (2026-08-03 재검증) | 전부 git/코드 검증값 — 제출 직전 재검증 커맨드는 포폴 작성 노트 참조. 도메인 수는 `app/domain/*/` 에서 `__pycache__` 제외 |
