...
1. **워크스루 3·4·6 알맹이 직격 (모드 전환)** — 9단계 정독 대신 어려운 함수만 직격하기로 합의(사용자 "봐야 할 것만 줄여서"). **난이도 실측(라인수)**: portfolio 1412(svc872+repo540) ≫ transaction 1048 ≫ account 602 ≫ 나머지 다 잔챙이. 알맹이는 3·4·6에 집중, 1·2·7 + category/fixed/stats/home/wealth는 CRUD/조회/합성이라 가볍게. **진짜 봐야 할 핵심 함수**: ▸account `_calc_balance`/`_calc_cash_balance`/`_calc_investment_balance`(잔액 컬럼 없이 매번 거래 합산, 현금/투자 분기) ▸transaction `list_account_ledger`+`_ledger_*`(원장 running balance를 커서 페이징으로) / `_signed_amount`(5타입→계좌 기준 부호) ▸portfolio `_recalc_item_from_transactions`(평단 재계산-과거매매 수정시 replay) / `_recompute_realized_pnl`(실현손익). **합의 순서: account→transaction→portfolio(쉬운→어려운 계단식)**. 다음 세션은 3번 account 잔액 동적계산부터.
