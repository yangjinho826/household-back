...
**로드맵 1번 A~C + D2 완결 (2026-07-27) — 전체 32 passed(멱등성 14+3 / 스케줄러 8 / smoke 3 / 도메인 4), 멱등성 97% · 락 로직 100%. D2 에서 A~C 최초의 진짜 소스 결함 발견·수정. 다음 D1 ledger.**
...
2. ~~**B advisory lock**~~ **완료**(8 케이스). ~~**C fault-injection**~~ **완료**(3 케이스, `test_crash_window.py`, 계획 `~/.claude/plans/c-dazzling-flamingo.md`). ~~**D2 realized_pnl**~~ **완료**(4 케이스, 결함 1건 수정). **다음: D1 ledger running balance** — 잔액을 저장 않고 desc 역산(`transaction/service.py:90-133`) + 페이지 경계는 커서에 `carry_balance` 를 실어 이어붙임(`:438`) → D2 와 같은 "계산 결과" 성격이라 실측 가치 있음. 이후 선택: D4 종목 수량/평단 상태 전이(매도>매수 차단·소멸/부활), D3 IDOR(회귀 안전망).
